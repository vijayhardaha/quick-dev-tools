...
const removeTrailingSlash = (path) => (path.slice(path.length - 1) === '/' ? path.slice(0, path.length - 1) : path);

export const findCurrentIndex = (pathname) => TOOLS.findIndex(({ link }) => removeTrailingSlash(pathname) === removeTrailingSlash(link));

export const getMeta = (pathname) => TOOLS.find(({ link }) => removeTrailingSlash(pathname) === removeTrailingSlash(link));
